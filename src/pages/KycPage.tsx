...
const KycPage: React.FC<KycPageProps> = ({ isEmbedded = false }) => {
  ...
  const handleKycSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(resData.message || 'Verification submission failed');
      }
  ...
  if (isPageLoading) {
    return (
      <div className={`${isEmbedded ? 'py-12 bg-transparent' : 'min-h-screen bg-gray-50'} flex flex-col items-center justify-center`}>
          ...
          {kycStatus === 'REJECTED' && (
            <div className="space-y-6">
              ...
              <div className="pt-4 flex gap-3 justify-center max-w-sm mx-auto">
                <Button variant="teal" className="w-full" onClick={handleRetry}>
                  Retry Verification
                </Button>
              </div>
            </div>
  ...
  if (isEmbedded) {
    return (
      <div className="max-w-3xl w-full mx-auto px-4 py-2 flex flex-col justify-center animate-fade-in">
        {kycContent}
      </div>
    );
  }
